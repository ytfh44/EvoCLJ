(ns evoclj.runtime.subagent
  ...
  the broker with the child's attenuated leases."
   (:require [clojure.java.jdbc :as jdbc]
             ...
             [evoclj.runtime.subagent-delivery :as delivery]
...
;; Plans (F1)
;; ---------------------------------------------------------------------------
;; The Delivery record moved to evoclj.runtime.subagent-delivery with the
;; cluster it belongs to. CancelPlan is the cancel plan (unused here); SpawnPlan
;; is this namespace's.
    ...
    (let [child-work-id (delivery/resolve-child-work-id! db child-id work-id)]
      ...
      (let [executor (build-child-executor db child)
            ...
            delivery (when (and parent-id child-work-id)
                       (delivery/auto-deliver-child-terminal! db parent-id child-id child-work-id))]
        (cond-> result
...
(defn run-subagent-async!
  ...
  (let [child-id (types/session-id child-session-id)
        child-work-id (delivery/resolve-child-work-id! db child-id work-id)
...
;; ---------------------------------------------------------------------------
;; S5 delivery — re-exported from evoclj.runtime.subagent-delivery
;; ---------------------------------------------------------------------------
;; The delivery cluster now lives in its own namespace. These aliases keep the
;; long-standing evoclj.runtime.subagent entry points working for callers that
;; already hold this namespace (the intent pipeline and the delivery tests).

(defn deliver-result-for-works!
  "Delegates to evoclj.runtime.subagent-delivery/deliver-result-for-works!."
  [db parent-session-id parent-work-id child-work-id terminal-event-id cas-ref]
  (delivery/deliver-result-for-works! db parent-session-id parent-work-id child-work-id terminal-event-id cas-ref))

(defn deliver-failure-for-works!
  "Delegates to evoclj.runtime.subagent-delivery/deliver-failure-for-works!."
  ([db parent-session-id parent-work-id child-work-id terminal-event-id]
   (delivery/deliver-failure-for-works! db parent-session-id parent-work-id child-work-id terminal-event-id))
  ([db parent-session-id parent-work-id child-work-id terminal-event-id opts]
   (delivery/deliver-failure-for-works! db parent-session-id parent-work-id child-work-id terminal-event-id opts)))

(defn deliver-result!
  "Delegates to evoclj.runtime.subagent-delivery/deliver-result!."
  [db parent-session-id child-session-id cas-ref]
  (delivery/deliver-result! db parent-session-id child-session-id cas-ref))

(defn deliver-failure!
  "Delegates to evoclj.runtime.subagent-delivery/deliver-failure!."
  [db parent-session-id child-session-id error]
  (delivery/deliver-failure! db parent-session-id child-session-id error))
