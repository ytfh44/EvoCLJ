(ns evoclj.capability.evolution-tools
  ...
  tools: :evolution/evidence and :evolution/history.

  The LLM mutator retrieves its context through the tool-calling loop
  instead of prompt-rendered context. Both tools are READ-ONLY views
  over the kernel's durable evolution data — the frozen evidence pack
  (CAS) and the rejection history (lineage rows) — and both are
  PRINCIPAL-BOUND through the capability broker: a tool-call intent
  carries the requesting session's attribution (Global Constraint 20),
  and the broker authorizes it against a host-owned lease that binds
  ONE exact principal to the tool resource with :actions #{:invoke}
  (Global Constraint 9 — a visible, requestable tool is never itself
  a grant). The lease factory here is the v0 grant set a host mints
  for the mutator's principal.
  ...
  each requested call through the capability broker (the adapter itself
  holds no broker — Global Constraint 8)."
...
(def EvidenceArgsSchema
  "The :evolution/evidence args contract (closed): the :evidence/id
  (a canonical ArtifactId) or the :candidate/id (a uuid) of the
  candidate whose frozen pack is resolved — at least one."
  [:and
   [:map {:closed true}
...
(def EvidenceOutputSchema
  ...
  or the typed not-found shape. Open so future pack fields validate
...
(def HistoryOutputSchema
  "The :evolution/history result: a vector of history entry summaries."
  [:vector :map])
...
(defn- evidence-id-of
  ...
  (if-let [id (:evidence/id args)]
    ...
    (let [row (first (sqlite/query (:sqlite store)
                                   ["SELECT evidence_id FROM candidates WHERE id = ?"
...
(defn evidence-provider
  "Build the kernel-owned READ-ONLY :evolution/evidence provider.
...
(defn history-provider
  "Build the kernel-owned READ-ONLY :evolution/history provider.
