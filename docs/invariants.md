...
- **Enforcement.** Landed by WO-W1/W2 (`runtime/work.clj` 7-state vocabulary + `verify-work-sm` + `collapse-ratio`, `store/work.clj` 5 CAS helpers + deadline sweeper, `018-work.sql`, `store/recovery.clj` → `store/work` recovery): `evoclj.runtime.work-test` (`work-collapses-48-to-7`, `work-sm-*`, `sweep-expired-deadlines-*`) + `test/evoclj/store/work_property_test.clj` (100 rounds per SM walk) assert `48 → 7` and acyclic/terminals; `evoclj.store.recovery-test` asserts orphan `queued` stays and `running|waiting` → `failed` with `:recovery/orphaned`. Mechanical guard: `scripts/verify-forms.clj` [W-19..W-25] verify product collapse; new production paths must write `works` (no `commands` module remains) and no bare `future` is exposed as API.

### INV-13 — Hydration pin → ExecutionHandle & Event prev + causal-links refinement (H1+E1)

`hydrate(pin) → ExecutionHandle` (H1) is the single construction path from a pinned session (`code_image_id/deployment_id/execution_id`/`generation_id`) to a fresh execution context (SCI, usage atom, CAS temp dir, DB-truth leases); unknown pin fails closed with `:hydrate/pin-not-found` (never synthetic). Events use **E1 split**: `prev/event-id` is the linear predecessor in the same session (nil only for `:session/created`, otherwise immediately preceding `seq = new-seq -1`, validated to stay local), `causal-links #{ {:from :type} }` is the cross-session semantic graph (any session `from` must exist, cross-session allowed); the `:cause/event-id` alias is removed from the append path (`store/event.clj` rejects it — no compat track accepts a cause id anymore). `verify-event-chain` checks positional `seq` continuity (`events[i].seq = i+1`), linear `prev` linkage, graph edge existence, and sha256 header digest.
