...
* **Refinement mapping:**
  ...
  * Legacy `:cause/event-id` alias removed from the append path (`store/event.clj` rejects it); the former `commands` compat track that still accepted a cause id was removed with `store/command.clj` (ExtraModules repair). New code must use `prev`+`causal-links`.
...
* **Break compat:** V1 intentionally breaks compat with heritage dual-anchor/cause/Command; no `subject→principal` or `cause→prev` canonicalization shims exist (the closed lease schema rejects `:subject`; the append path rejects `:cause/event-id`) — compat is storage-level only (`capabilities` `subject_*` backfill columns; old `commands` rows are inert history). New code must use Principals, Grant meet, Work, prev+causal-links, and H1 hydration (verified by `scripts/verify-forms.clj` refinement che...
...
| Subagent+Event refinement | `src/evoclj/intent/schema.clj`, `intent/dispatch.clj`, `src/evoclj/runtime/subagent.clj` (Work child via `works.parent_work_id`), `src/evoclj/runtime/hydrate.clj` (H1), `src/evoclj/store/session.clj` (immutable pin I1), `src/evoclj/store/recovery.clj` (orphans via Work), `src/evoclj/store/event.clj` + `event_schema.clj` (E1 prev/causal-links), `resources/migrations/017-event-prev-causal-links.sql` | 8-state Session SM retired |
| Work+Event chain | `src/evoclj/runtime/work.clj` (vocabulary 7 + product collapse), `src/evoclj/store/work.clj` (schema + SM CAS + W2 recovery + deadline sweeper), `src/evoclj/store/event.clj` (append, seq, prev, causal-links, hash), `src/evoclj/store/event_schema.clj`, `src/evoclj/store/migrate.clj`, `src/evoclj/runtime/hydrate.clj`, `resources/migrations/014-code-image-deployment-execution.sql` (I1), `018-work.sql` (W1) | `store/command.clj` removed (6-state heritage deleted; table inert in old DBs) |
