...
- **Scope:** 13 e2e acceptance tests mapped to closure items + 4 tracked baseline-failure namespaces + BT11 out-of-scope set + the 2 retired-ledger doc-hash agent-ids (formerly tracked in the repair ledger, since removed).
- **Constraint honored:** no `src/` changes, no edits to the repair ledger, no git checkout/stash, no commit, no mutation of the tree beyond this new untracked report.

---

## 1. The 13 e2e tests → production entry point → evidence

The repair ledger's e2e map ("e2e 映射") is `#1→M20 #2→M1 #3→M12 #4→M13 #5→M15 #6/#7→S1 #8→B1 #9→S4 #10→S5 #11→E2 #12→S10 #13→D1`. Each acceptance namespace drives the real production entry point listed below. All runs are per-namespace and the counts are the exact cognitect output.
...
[WO-T5 path-5] real docs/: 28 files, 74 unique refs, exit 1
  INVALID <retired-repair-ledger> : 125 T1-impl-1
  INVALID <retired-repair-ledger> : 125 T1-impl-2
```

These are the two ledger **agent-instance-ids** (not commit refs) recorded as `D1-followup` — i.e. the remaining invalid refs are exactly the two retired-ledger doc-hash agent-ids, matching the D1 note (the ledger entry reads "仅剩 2 个…agent-instance-id(裁定非 commit 引用·D1-followup)"). INV-08: the current-closure-report share is closed; these 2 are adjudicated non-commit refs, tracked as out-of-scope. (The scan printed the ledger's then-path; the ledger file has since been retired, hence the redaction above.)
...
| doc-hash agent-ids | retired repair ledger (:125) | 2 | 2 | `T1-impl-1` + `T1-impl-2` — non-commit agent-instance-ids, D1-followup |
...
- D1-followup: the 2 retired-ledger (:125) agent-instance-ids (T1-impl-1 / T1-impl-2).
...
*Report produced by implementer A for item D2. No commit made; `docs/D2-closure-report.md` is a new untracked file. `src/` and the repair ledger untouched.*
