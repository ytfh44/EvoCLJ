...
(defn- lease [tool-id]
  {:cap/id (random-uuid) :principal {:principal/type :session :session/id sid}
...
(deftest mcp-filesystem-second-authorization-holds
  (testing "even when tool is visible, filesystem/path resource still requires its own lease"
    (let [reg (registry/create-registry)
          ...
          tool-lease {:cap/id (random-uuid) :principal {:principal/type :session :session/id sid}
      ...
      (let [path-lease {:cap/id (random-uuid) :principal {:principal/type :session :session/id sid}
