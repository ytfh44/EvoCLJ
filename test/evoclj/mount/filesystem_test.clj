...
(deftest same-provider-serves-both
  (testing "single FilesystemProvider serves workspace and skill mount"
    (let [ws-dir (temp-dir)
          ...
          principal {:principal/type :session :session/id #uuid "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa"}
          ws-lease {:cap/id (random-uuid) :principal principal :resource {:kind :filesystem/path :mount/id [:workspace "ws"] :path ""} :actions #{:read :list :stat :write :create :delete} :issued-at now :expires-at (Date. (+ (.getTime now) 100000)) :constraints {}}
          skill-lease {:cap/id (random-uuid) :principal principal :resource {:kind :filesystem/path :mount/id [:skill "demo" (:tree/id snap-res)] :path ""} :actions #{:read :list :stat} :issued-at now :expires-at (Date. (+ (.getTime now) 100000)) :constraints {}}]
      ;; skill read via provider
      (let [content (fs/provider-read provider [:skill "demo" (:tree/id snap-res)] "SKILL.md" {:leases [skill-lease] :principal principal :now now})]
        (is (= "skill" (String. content))))
      ;; workspace read via same provider
      (let [content (fs/provider-read provider [:workspace "ws"] "a.txt" {:leases [ws-lease] :principal principal :now now})]
        (is (= "workspace" (String. content))))
      ;; skill write must fail even with RW lease
      (let [rw-leases (conj [skill-lease] {:cap/id (random-uuid) :principal principal :resource {:kind :filesystem/path :mount/id [:skill "demo" (:tree/id snap-res)] :path ""} :actions #{:write :create} :issued-at now :expires-at (Date. (+ (.getTime now) 100000)) :constraints {}})]
        (is (thrown? clojure.lang.ExceptionInfo (fs/provider-write provider [:skill "demo" (:tree/id snap-res)] "SKILL.md" (.getBytes "x") {:leases rw-leases :principal principal :now now}))))
      ;; workspace write requires both surface and lease
      (is (thrown? clojure.lang.ExceptionInfo (fs/provider-write provider [:workspace "ws"] "new.txt" (.getBytes "x") {:leases [skill-lease] :principal principal :now now})))
      (let [res (fs/provider-create provider [:workspace "ws"] "new.txt" (.getBytes "new") {:leases [ws-lease] :principal principal :now now})]
        (is (some? res)))
      ;; .. cannot escape
      (is (thrown? clojure.lang.ExceptionInfo (fs/provider-read provider [:skill "demo" (:tree/id snap-res)] "../etc/passwd" {:leases [skill-lease] :principal principal :now now})))
      ;; mount A lease cannot access mount B
      (is (thrown? clojure.lang.ExceptionInfo (fs/provider-read provider [:workspace "ws"] "a.txt" {:leases [skill-lease] :principal principal :now now})))
      ;; CAS snapshot independence
      (write-file skill-dir "SKILL.md" "changed")
      (let [content2 (fs/provider-read provider [:skill "demo" (:tree/id snap-res)] "SKILL.md" {:leases [skill-lease] :principal principal :now now})]
...
(defn- skill-read-fixture
  ...
  {:provider p :mount-id id :lease lease :principal principal}."
  []
  (let [skill-dir (temp-dir)
        ...
        provider (fs/make-provider reg)
        principal {:principal/type :session :session/id #uuid "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa"}
        lease {:cap/id (random-uuid) :principal principal :resource {:kind :filesystem/path :mount/id mount-id :path ""} :actions #{:read :list :stat} :issued-at now :expires-at (Date. (+ (.getTime now) 100000)) :constraints {}}]
    {:provider provider :mount-id mount-id :lease lease :principal principal}))
...
(deftest read-skill-file-facade-deleted-and-provider-read-intact
  ...
  (testing "equivalence-to-deletion: the delegated production reader still serves a valid skill file"
    (let [{:keys [provider mount-id lease principal]} (skill-read-fixture)]
      (is (= "skill" (String. (fs/provider-read provider mount-id "SKILL.md" {:leases [lease] :principal principal :now now}))))))
  (testing "fault 1: a missing file is rejected with the typed :filesystem/not-found error"
    (let [{:keys [provider mount-id lease principal]} (skill-read-fixture)]
      (is (= :filesystem/not-found
             (err-type #(fs/provider-read provider mount-id "no-such.md" {:leases [lease] :principal principal :now now}))))))
  (testing "fault 2: a path escaping the skill mount is rejected with the typed :filesystem/path-outside-mount error"
    (let [{:keys [provider mount-id lease principal]} (skill-read-fixture)]
      (is (= :filesystem/path-outside-mount
             (err-type #(fs/provider-read provider mount-id "../etc/passwd" {:leases [lease] :principal principal :now now})))))))
