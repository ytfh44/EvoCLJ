...
(deftest fresh-database-applies-all-migrations
  (let [db (sqlite/spec (temp-db-path))]
    (is (= {:status :applied :version 24} (migrate/migrate! db)))
    (is (= 24 (migrate/current-version db)))
...
(deftest second-apply-is-a-safe-noop
  (let [db (fresh-db)
    ...
    (is (= {:status :noop :version 24} (migrate/migrate! db)))
...
(deftest true-legacy-stamped-five-applies-pending-006
  ...
  (let [db (fresh-db)
    ...
    (testing "all pending migrations run; status :applied at version 24"
      (is (= {:status :applied :version 24} result)))
...
(deftest version-ahead-of-code-fails-cleanly
  ...
  (let [db (fresh-db)
        _ (set-meta! db "schema_version" "25")
        e (migrate-error db)]
    (is (some? e))
    (is (= :store/schema-mismatch (:error/type (ex-data e))))
    (is (= :version-ahead (:reason (ex-data e))))
    (is (= 25 (:actual (ex-data e))))
    (testing "the failed attempt changed nothing"
  ...
  (let [chain-error
    ...
    (testing "a chain whose top disagrees with latest-version"
      (let [e-short (chain-error (butlast full-chain))     ; tops out at 22
            e-long (chain-error (conj full-chain "025-beyond.sql"))] ; tops out at 25 (drift)
...
(deftest version-1-database-upgrades-additively
  ...
  ;; migration and bring the version forward to the true latest (23,
