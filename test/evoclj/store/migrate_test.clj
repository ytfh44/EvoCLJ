...
(deftest fresh-database-applies-all-migrations
  (let [db (sqlite/spec (temp-db-path))]
    (is (= {:status :applied :version 16} (migrate/migrate! db)))
    (is (= 16 (migrate/current-version db)))
...
(deftest second-apply-is-a-safe-noop
  (let [db (fresh-db)
    ...
    (is (= {:status :noop :version 16} (migrate/migrate! db)))
...
(deftest true-legacy-stamped-five-applies-pending-006
  ...
  (let [db (fresh-db)
    ...
    (testing "only 006-009 runs; status :applied at version 9"
      (is (= {:status :applied :version 16} result)))
...
(deftest version-ahead-of-code-fails-cleanly
  ...
  (let [db (fresh-db)
        _ (set-meta! db "schema_version" "17")
        e (migrate-error db)]
    (is (some? e))
    (is (= :store/schema-mismatch (:error/type (ex-data e))))
    (is (= :version-ahead (:reason (ex-data e))))
    (is (= 17 (:actual (ex-data e))))
    (testing "the failed attempt changed nothing"
      (is (= "17" (meta-value db "schema_version"))))))
...
(def ^:private full-chain
  ["001-init.sql" "002-memory.sql" "003-routing.sql"
   ...
   "007-singleton-current.sql" "008-normalize-candidate.sql" "009-cas-fk-existence.sql" "010-promotion-outbox.sql" "011-session-memory-fk.sql" "012-commands.sql" "013-capabilities.sql" "014-code-image-deployment-execution.sql" "015-principal.sql" "016-resource-edn.sql"])
...
(deftest broken-migration-chains-fail-closed
  (let [chain-error
    ...
    (testing "a chain whose top disagrees with latest-version"
      (let [e-short (chain-error (butlast full-chain))     ; tops out at 15
            e-long (chain-error (conj full-chain "017-beyond.sql"))] ; tops at 17
